...
// Enhanced Intersection Observer for scroll-triggered animations with stagger animation support
function createScrollAnimations() {
  ...
  // Main animation observer for single elements and scroll-reveal
  const observer = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
      if (entry.isIntersecting) {
        const target = entry.target;

        // Handle scroll-reveal class with custom animation
        if (target.classList.contains('scroll-reveal')) {
          target.classList.add('animate');
          // Don't unobserve, allow re-triggering
        } else {
          // Add base animation class for other elements
          ...
          // Stop observing once animated (except for scroll-reveal elements)
          observer.unobserve(target);
        }
      } else {
        // Reset scroll-reveal animations when elements leave viewport
        if (target.classList.contains('scroll-reveal')) {
...
// Advanced Micro-Interaction System for Enhanced User Feedback
function initializeAdvancedMicroInteractions() {
  console.log('🎨 Initializing Advanced Micro-Interaction System...');

  // Smart cursor follower system
  createSmartCursorFollower();
  // Advanced ripple effects with custom configurations
  createCustomRippleEffects();
  // Interactive particle system for special elements
  createInteractiveParticles();
  // Gesture-based micro-interactions
  initializeGestureInteractions();
  // Enhanced hover feedback system
  createAdvancedHoverEffects();
  // Notification micro-animations
  createNotificationMicroAnimations();
  // Dynamic feedback sounds (visual only for now)
  initializeVisualAudioFeedback();
  // Haptic simulation for touch devices
  createHapticSimulation();

  console.log('✅ Advanced Micro-Interactions Initialized');
}

// Smart Cursor Follower - Removed for cleaner UX
// Function archived to prevent distracting mouse-following interactions

// Custom Ripple Effects with Advanced Configurations
function createCustomRippleEffects() {
  // Enhanced ripple effect for all interactive elements
  document.addEventListener('click', function(e) {
    const target = e.target;
    if (target.matches('button, .btn-enhanced, .ripple-effect, .interactive-element')) {
      createAdvancedRipple(e, target);
    }
  });
  ...
  function createAdvancedRipple(event, element) {
    ...
    ripple.style.cssText = `
      ...
      transform: scale(0);
      animation: advanced-ripple 0.6s ease-out;
      pointer-events: none;
      z-index: 10;
    `;
    ...
    if (element.dataset.rippleColor) {
      ripple.style.background = element.dataset.rippleColor;
    }
...
function createInteractiveParticles() {
  ...
  let ctx, particles = [], mouse = { x: -1000, y: -1000 };
  ...
  function initParticles() {
    ...
    for (let i = 0; i < 50; i++) {
      particles.push({
        ...
        color: `rgba(${59 + Math.random() * 40}, ${130 + Math.random() * 40}, ${246}, ${0.5})`
      });
    }
  ...
  function animateParticles() {
    ...
    particles.forEach(particle => {
      ...
      // Interactive behavior near mouse
  ...
  // Toggle particles based on special classes
  function checkForParticleElements() {
    const particleElements = document.querySelectorAll('.particle-trigger');
    if (particleElements.length > 0 && !document.body.contains(particleCanvas)) {
      document.body.appendChild(particleCanvas);
      initParticles();
    }
  }

  // Listen for particle trigger elements
  const observer = new MutationObserver(checkForParticleElements);
  observer.observe(document.body, { childList: true, subtree: true });

  checkForParticleElements();
}
...
function initializeGestureInteractions() {
  ...
  document.addEventListener('touchend', (e) => {
    ...
    if (distance > 50 && deltaTime < 500) {
      ...
      if (target && target.matches('.gesture-responsive')) {
  ...
  function handleSwipeGesture(element, gesture) {
    // Visual feedback for swipe
    const feedback = document.createElement('div');
    feedback.textContent = `→ ${gesture.direction.toUpperCase()}`;
    feedback.style.cssText = `
      ...
      font-weight: bold;
      z-index: 10000;
      animation: swipe-feedback 1s ease-out;
      pointer-events: none;
    `;
...
function createAdvancedHoverEffects() {
  ...
  document.querySelectorAll('.magnetic-hover').forEach(element => {
    element.addEventListener('mousemove', function(e) {
      ...
      this.style.transform = `translate(${deltaX * 0.1}px, ${deltaY * 0.1}px) scale(1.05)`;
    });
  ...
  document.querySelectorAll('.hover-glow').forEach(element => {
    element.addEventListener('mouseenter', function() {
      this.style.boxShadow = '0 0 30px rgba(11, 61, 145, 0.5)';
    });
  ...
  document.querySelectorAll('.scale-pulse').forEach(element => {
    element.addEventListener('click', function() {
      this.style.animation = 'scale-pulse-animation 0.2s ease';
      setTimeout(() => {
  ...
  // CSS for scale pulse
  if (!document.querySelector('#scale-pulse-styles')) {
    const style = document.createElement('style');
    style.id = 'scale-pulse-styles';
    style.textContent = `
...
// Notification Micro-Animations
function createNotificationMicroAnimations() {
  window.showNotificationFeedback = function(message, type = 'info') {
    const notification = document.createElement('div');
    notification.className = `notification-feedback notification-${type}`;
    ...
    notification.style.cssText = `
      ...
      gap: 0.75rem;
      z-index: 10000;
      animation: notification-slide-in 0.5s cubic-bezier(0.4, 0, 0.2, 1);
  ...
  if (!document.querySelector('#notification-styles')) {
    ...
    style.textContent = `
      ...
      .notification-info { border-left: 4px solid #0b3d91; }
    `;
    document.head.appendChild(style);
  }
}

// Visual Audio Feedback System - Removed for cleaner UX
// Function moved to archive to prevent distracting interactions

// Haptic Feedback Simulation
function createHapticSimulation() {
  // Simulate haptic feedback with visual cues
  window.simulateHaptic = function(intensity = 'medium', duration = 100) {
    const haptic = document.createElement('div');
    haptic.style.cssText = `
      position: fixed;
      top: 0;
      left: 0;
      width: 100%;
      ...
      animation: haptic-feedback ${duration}ms ease-in-out;
      pointer-events: none;
      z-index: 9999;
    `;

    document.body.appendChild(haptic);
    setTimeout(() => haptic.remove(), duration);
  };

  // Auto haptic feedback for interactive elements
  document.addEventListener('click', (e) => {
    if (e.target.matches('.btn-enhanced, button, .haptic-feedback')) {
      simulateHaptic('light', 50);
    }
  });

  document.addEventListener('touchstart', (e) => {
    if (e.target.matches('.btn-enhanced, button, .haptic-feedback')) {
      simulateHaptic('medium', 75);
    }
  }, { passive: true });

  // CSS animation
  if (!document.querySelector('#haptic-styles')) {
    const style = document.createElement('style');
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Removed overly complex micro-interactions for cleaner UX
